  ...
  for (const [language, translations] of languages.entries()) {
    for (const [key, translation] of translations.entries()) {
      ...
      for (const input of translationInputs) {
        ...
        fnParamsTypes += `${input[1].replace(/\{|\}/g, '').trim()}: string | number, `;
      }
      ...
      translationFiles.add(key);
    }
  }

  for (const [key] of translationFiles.entries()) {
    ...
    translationContent += `export const ${key} = (inputs: ${translationTypesAsArray.length > 0 ? `{ ${translationTypesAsArray.map(type => `${type}: string | number`).join(', ')} }` : '{} = {}'}, locale?: string) => {
